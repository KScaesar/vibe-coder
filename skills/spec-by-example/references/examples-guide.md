...
- **US-Client-1**：作為呼叫方，為了逾時後可以直接重試，我想要重送同一筆請求時不會產生第二張訂單
- **US-Client-2**：作為呼叫方，為了不必為重試寫特別的處理分支，我想要重送時拿到與第一次完全相同的回應
- **US-Client-3**：作為呼叫方，為了及早發現自己 key 產生邏輯的 bug，我想要用同一把 key 送不同內容時被明確擋下
- **US-Client-4**：作為呼叫方，為了決定重試視窗要開多長，我想要知道 key 的有效期限
- **US-Operator-1**：作為維運，為了不必人工對帳，我想要併發重送不會建出兩張訂單
- **US-Operator-2**：作為維運，為了儲存成本可控，我想要冪等記錄有保存上限
- **US-Operator-3**：作為維運，為了決定要不要降級放行，我想要冪等層故障時的行為是可預期的
